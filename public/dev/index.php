...
<html>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="btn-group" style="float: right; padding: 2%">
                        ...
                        <button class="btn btn-primary btn-lg" id="save">Preview</button>
                        <input type="checkbox" name="af_checkbox" id="activate_form"; style="display: none">
                </div>
            </div>
        </div>
    </div>
	...
	/* Specific method to save Options and their corresponding Part Numbers */
		...
		*/ 
		  	...
		  	/* WHERE I LEFT OFF - MUST RESET THE SECTION INDEX EVERYTIME A CATEGORY IS DROPPED */
		  			...
		  			$.each(array_sects, function(index, sects_obj) {
						...
						if (($section.prop('type')) == "section") {
							...
							// console.log("model", model);		
							// $new_32.10area.find('.droppedSect').find('.ctrl-section').last().html(sects_obj.label);
						}
		...
		// $("#work-area").children().find('.droppedElem').each(function(){
		// 			var id_generator = $(this);
		// 			console.log(id_generator);
		// 			id_generator[0].id = "CTRL-DIV-"+(counter++);
		// 			var name = id_generator[0].id;
		// });
		...
		$('#save').on('click', function() {
		  ...
		  dialogContent+= 		'<button type="button" id="test-toggle" class="btn btn-info btn-lg">Test Toggle</button>';		  
		  dialogContent+=	'</div>';
		  dialogContent+= '</div>';
		  dialogContent+= '<br>';
		  dialogContent+= '<div class="row">';
		  dialogContent+= 	'<div class="col-md-9" style="margin-left: 15px">';
		  dialogContent+= 		'<div id="toggle-pairs" style="border: 1px solid #ccc; padding: 5px; margin: 5px">';
		  dialogContent+=		'</div>';
		  dialogContent+=	'</div>';
		  dialogContent+= '</div>';		  
		  ...
		  dialogContent+= 'function output(inp) {document.body.appendChild(document.createElement("pre")).innerHTML = inp;}';
		  dialogContent+= '$(\'#test-toggle\').on("click", function() {\
		  					 \n $(\'#toggle-pairs\').toggle();\
							 \n var new_obj = {};\
