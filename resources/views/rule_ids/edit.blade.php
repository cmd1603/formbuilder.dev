...
<div class="container" style="margin-bottom: 20px">
	...
	<form method="POST" action="{{ action('RuleIdsController@update', $rule_id->id) }}" enctype="multipart/form-data">		
	{{ method_field('PUT') }}
	{!! csrf_field() !!}
	<div class="row">
		<div class="row">
	        <div><h4>Configuration</h4>
				<input type="text" class="form-control" id="dynamic_dropdown" name="directory_label" value="{{ $rule_id->directory_label }}" readonly>
        	</div>			
	        <div><h4>Rule Name</h4>
				<input type="text" class="form-control" id="rule_name" name="rule_name" value="{{ $rule_id->rule_name }}" readonly>
	        </div>	            
		</div>
		<br>
		<div class="row" id="table_wrappers">
		</div> 
	   	<div><button type="button" class="btn btn-success" id="add_row_edit">Add</button></div>   				
	...
	<div class="row">
		...
		<table class="table table-bordered tab_logic" id="rule_record_table">
			<thead data-id="0">
				<tr>
					<th class="text-center" id="id_header">#</th>
...
<!--    	<div><button type="submit" class="btn btn-primary" id="submit_rule">Submit</button></div> -->
   	</form>		
</div>
			...
			<div class="modal-body">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<p>
							<label>Output</label>
								<input class="form-control output_1" type="text" name="output_1">
						</p>	
					</div>						
				</div>
...
$(document).ready(function () {
		...
		$.ajax({
			...
			success:function(data) {
				...
				$.each(data, function(index, item) {
					if ($(this).prop('directory_label') == dr_label) {			
				...
				input_names_array.push(extracted_part_numbers);
		  		...
		  		$(".inp1_1").append('<option value="">Please make a selection</option>'  + '<option value="r(">' + "Required" + "</option>");
		    	...
		    	$('.sn1 option, .sn1_1 option, .sn2 option:last-child, .sn2_1 option:last-child, .sn3 option:last-child, .sn3_1 option:last-child, .s_output option:last-child, .s_output_1 option:last-child, .inp1 option:last-child, .inp1_1 option:last-child,  .inp2 option:last-child, .inp2_1 option:last-child, .inp3 option:last-child, .inp3_1 option:last-child').each(function() {
		    		if ($(this).text() == "") {
		    			$(this).remove();
		    		}
		    	});	
	...
	// ------- LOADS IN THE RULES DATA FOR THE TABLE ON EDIT PAGE -------- //
	$.ajax({
		type: 'get',
		...
		success:function(data) {
			console.log(data);
			$.each(data, function(index, item) {
				if($(this).prop('rule_name') == rule_name) {
					console.log(rule_name);
					$('#rule_record_table').append(
						"<tr class='item" + $(this).prop("id") + "'>\
							<td class=\"tb_data did\">" + $(this).prop("id") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("submitted_name_1") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("input_1") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("submitted_name_2") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("input_2") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("submitted_name_3") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("input_3") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("submitted_output") + "</td>\
							<td class=\"tb_data\">" + $(this).prop("output") + "</td>\
							<td class=\"tb_data\">\
	...
	// ------- SAVES THE ADDED RECORDS TO THE EXISTING TABLE ON EDIT PAGE -------- //
	...
	$(document).on("click", ".edit-modal", function() {
		...
		$(".s_output_1").children().each(function() {
			if($(this).text() == saved_sub_otpt) {
				...
				// $(this).attr('selected', false); Tony doesn't want this changed just yet
			}
		});
	...
	function sortTable(f,n) {
		var rows = $('#rule_record_table tbody tr').get();
		...
		$.each(rows, function(index, row) {
			$('#rule_record_table').children('tbody').append(row);
		});
	}

	var f_sl = 1;

	$('#id_header').on('click', function() {
		f_sl *= -1;
